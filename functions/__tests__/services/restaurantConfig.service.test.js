...
describe('RestaurantConfigService', () => {
    ...
    describe('getPhoneNumber', () => {
        it('retrieves a phone number successfully', async () => {
            ...
            const mockPhoneNumberRef = { get: jest.fn().mockResolvedValue(mockDocumentSnapshot('phoneNumber', phoneNumberData)) };
            db.collection.mockReturnValue({ doc: jest.fn().mockReturnValue(mockPhoneNumberRef) });

            const result = await RestaurantConfigService.getPhoneNumber();

            expect(result).toEqual(PhoneNumber.fromFirestore(mockDocumentSnapshot('phoneNumber', phoneNumberData)));
        });
    ...
    describe('getEmail', () => {
        it('retrieves an email successfully', async () => {
            ...
            const mockEmailRef = { get: jest.fn().mockResolvedValue(mockDocumentSnapshot('email', emailData)) };
            db.collection.mockReturnValue({ doc: jest.fn().mockReturnValue(mockEmailRef) });
    ...
    describe('addAddress', () => {
        it('adds an address successfully', async () => {
            const addressData = { street: '123 Main St', city: 'Anytown', county: 'Anycounty', eircode: '12345', country: 'Anyland' };
            ...
            await RestaurantConfigService.addAddress(addressData.street, addressData.city, addressData.county, addressData.eircode, addressData.country);
    ...
    describe('updateAddress', () => {
        it('updates an address successfully', async () => {
            ...
            await RestaurantConfigService.updateAddress(addressData.street, addressData.city, addressData.county, addressData.eircode, addressData.country);
    ...
    describe('getAddress', () => {
        it('retrieves an address successfully', async () => {
            const addressData = { street: '123 Main St', city: 'Anytown', county: 'Anycounty', eircode: '12345', country: 'Anyland' };
            const mockAddressRef = { get: jest.fn().mockResolvedValue(mockDocumentSnapshot('address', addressData)) };
            db.collection.mockReturnValue({ doc: jest.fn().mockReturnValue(mockAddressRef) });
    ...
    describe('getRestaurantConfig', () => {
        it('retrieves the complete restaurant configuration successfully', async () => {
            ...
            jest.spyOn(RestaurantConfigService, 'getPhoneNumber').mockResolvedValue(PhoneNumber.fromFirestore(mockDocumentSnapshot('phoneNumber', phoneNumberData)));
            jest.spyOn(RestaurantConfigService, 'getEmail').mockResolvedValue(Email.fromFirestore(mockDocumentSnapshot('email', emailData)));
            jest.spyOn(RestaurantConfigService, 'getAddress').mockResolvedValue(Address.fromFirestore(mockDocumentSnapshot('address', addressData)));
            jest.spyOn(RestaurantConfigService, 'getMap').mockResolvedValue(Map.fromFirestore(mockDocumentSnapshot('map', mapData)));
            jest.spyOn(RestaurantConfigService, 'getAllFeatures').mockResolvedValue(featuresData.map(f => Feature.fromFirestore(mockDocumentSnapshot(f.name, f))));

            const result = await RestaurantConfigService.getRestaurantConfig();

            expect(result).toEqual({
                phoneNumber: PhoneNumber.fromFirestore(mockDocumentSnapshot('phoneNumber', phoneNumberData)),
