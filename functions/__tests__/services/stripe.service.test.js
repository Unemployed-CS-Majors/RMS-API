...
describe('Stripe Service', () => {
    ...
    describe('createPaymentIntent', () => {
        it('creates a payment intent successfully', async () => {
            const order = new Order(null, 'user1', [{
                id: 'item1',
                quantity: 2,
                price: 10.99
            }], 21.98, 1.76, 5.00, 28.74, 'home_delivery', 'online');
        ...
        it('throws an error if payment intent creation fails', async () => {
            ...
            await expect(StripeService.createPaymentIntent(order)).rejects.toThrow('Failed to create payment intent');
        });
    });
    ...
    describe('retrievePaymentIntent', () => {
        ...
        it('throws an error if payment intent retrieval fails', async () => {
            stripe.paymentIntents.retrieve.mockRejectedValue(new Error('Failed to retrieve payment intent'));

            await expect(StripeService.retrievePaymentIntent('pi_123')).rejects.toThrow('Failed to retrieve payment intent');
        });
    });
    ...
    describe('createCheckoutSession', () => {
        it('creates a checkout session successfully', async () => {
            ...
            const result = await StripeService.createCheckoutSession(order, 'https://success.url', 'https://cancel.url');
            ...
            expect(stripe.checkout.sessions.create).toHaveBeenCalledWith(expect.objectContaining({
                ...
                line_items: [{
                    ...
                    price_data: {currency: "usd", product_data: {name: "Delivery Fee"}, unit_amount: 500},
                    quantity: 1
                }],
        ...
        it('throws an error if checkout session creation fails', async () => {
            ...
            stripe.checkout.sessions.create.mockRejectedValue(new Error('Failed to create checkout session'));

            await expect(StripeService.createCheckoutSession(order, 'https://success.url', 'https://cancel.url')).rejects.toThrow('Failed to create checkout session');
        });
    });
    ...
    describe('retrieveCheckoutSession', () => {
        ...
        it('throws an error if checkout session retrieval fails', async () => {
            stripe.checkout.sessions.retrieve.mockRejectedValue(new Error('Failed to retrieve checkout session'));

            await expect(StripeService.retrieveCheckoutSession('cs_123')).rejects.toThrow('Failed to retrieve checkout session');
        });
    });
    ...
    describe('handleWebhookEvent', () => {
        it('handles a webhook event successfully', async () => {
            ...
            expect(stripe.webhooks.constructEvent).toHaveBeenCalledWith(payload, signature, undefined);
        });
        ...
        it('throws an error if webhook event handling fails', async () => {
            ...
            await expect(StripeService.handleWebhookEvent(payload, signature)).rejects.toThrow('Invalid signature');
        });
    });
});