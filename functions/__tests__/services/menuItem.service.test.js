...
const { db, storage, admin } = require('../../app/config/firebase.config');
...
describe('MenuItem Service', () => {
    ...
    describe('uploadImage', () => {
        it('uploads an image and returns the URL', async () => {
            ...
            expect(result).toBe('https://storage.googleapis.com/test-bucket/menu-items/unique-id_test.jpg');
        });
    ...
    describe('createMenuItem', () => {
        it('creates a new menu item with an image', async () => {
            ...
            const file = { originalname: 'test.jpg', buffer: Buffer.from('test'), mimetype: 'image/jpeg' };
            const mockDocRef = { id: 'menuItem1', get: jest.fn().mockResolvedValue(mockDocumentSnapshot('menuItem1', data)) };
    ...
    describe('getMenuItemById', () => {
        it('retrieves a menu item by its ID', async () => {
            ...
            const mockDocRef = { get: jest.fn().mockResolvedValue(mockDocumentSnapshot('1', mockMenuItem)) };
    ...
    describe('updateMenuItem', () => {
        it('updates a menu item with a new image', async () => {
            ...
            const mockMenuItem = { id: '1', name: 'Burger', price: 10.99, imageUrl: 'https://storage.googleapis.com/test-bucket/menu-items/old.jpg' };
            const mockDocRef = { get: jest.fn().mockResolvedValue(mockDocumentSnapshot('1', mockMenuItem)), update: jest.fn() };
            db.collection.mockReturnValue({ doc: jest.fn().mockReturnValue(mockDocRef) });
            uuidv4.mockReturnValue('unique-id');
            ...
            const result = await MenuItemService.updateMenuItem('1', data, file);
        });
        ...
        it('updates a menu item without a new image', async () => {
            ...
            const result = await MenuItemService.updateMenuItem('1', data, null);
        ...
        it('throws an error if the menu item does not exist', async () => {
            ...
            await expect(MenuItemService.updateMenuItem('1', { name: 'Updated Burger' }, null)).rejects.toThrow('Menu item not found');
        });
    });

    describe('deleteMenuItem', () => {
        it('deletes a menu item and its image', async () => {
            const mockMenuItem = { id: '1', name: 'Burger', price: 10.99, imageUrl: 'https://storage.googleapis.com/test-bucket/menu-items/test.jpg' };
            const mockDocRef = { get: jest.fn().mockResolvedValue(mockDocumentSnapshot('1', mockMenuItem)), delete: jest.fn() };
            db.collection.mockReturnValue({ doc: jest.fn().mockReturnValue(mockDocRef) });
            const fileRef = storage.bucket().file('menu-items/test.jpg');
            fileRef.exists.mockResolvedValue([true]);
            fileRef.delete.mockResolvedValue();
