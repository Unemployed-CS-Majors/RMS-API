...
describe('Reservation Controller', () => {
    ...
    describe('cancelReservation', () => {
        it('cancels a reservation successfully', async () => {
            ...
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ status: 'success', message: 'Reservation cancelled successfully' }));
        });
        ...
        it('returns 400 if reservation ID is missing', async () => {
            ...
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ status: 'error', message: 'Reservation ID is required' }));
        });
        ...
        it('returns 404 if reservation not found', async () => {
            ...
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ status: 'error', message: 'Reservation not found' }));
        });
        ...
        it('returns 404 if user not found', async () => {
            ...
            expect(res.status).toHaveBeenCalledWith(404);
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ status: 'error', message: 'User not found' }));
        });
        ...
        it('handles errors and returns 500 status', async () => {
            ...
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ status: 'error', message: 'Database error' }));
        });
    });
});