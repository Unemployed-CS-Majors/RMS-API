...
describe('Order Controller', () => {
    ...
    describe('updateOrderStatus', () => {
        it('updates an order status successfully', async () => {
            ...
            expect(OrderService.updateOrderStatus).toHaveBeenCalledWith('order1', 'IN_PROGRESS', expect.any(Object));
            expect(res.status).toHaveBeenCalledWith(200);
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
                status: 'success',
        ...
        it('handles errors and returns 500 status', async () => {
            ...
            expect(logger.error).toHaveBeenCalledWith('Error updating order status: Database error', error);
            expect(res.status).toHaveBeenCalledWith(500);
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
                status: 'error',
                message: 'Error getting order'
    ...
    describe('getActiveOrders', () => {
        ...
        it('handles errors and returns 500 status', async () => {
            ...
            expect(logger.error).toHaveBeenCalledWith('Error getting active orders: Database error', error);
            expect(res.status).toHaveBeenCalledWith(500);
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
                status: 'error',
                message: 'Error getting order'
