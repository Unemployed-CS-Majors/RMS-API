...
describe('Payment Controller', () => {
    ...
    describe('handlePaymentSuccess', () => {
        it('returns 400 if session_id is missing', async () => {
            ...
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: 'Missing session ID' }));
        });
        ...
        it('returns 500 on error', async () => {
            ...
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ status: 'error', message: 'Error while receiving order', data: error }));
        });
    });
    ...
    describe('handlePaymentCancel', () => {
        it('handles payment cancel and redirects to cancel page', async () => {
            ...
            expect(OrderService.updateOrderStatus).toHaveBeenCalledWith('order1', 'pending_payment', { paymentNote: 'Payment was canceled' });
        ...
        it('returns 500 on error', async () => {
            ...
            expect(logger.log).toHaveBeenCalledWith('error', `Error handling payment cancel: ${error.message}`);
            expect(res.status).toHaveBeenCalledWith(500);
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ status: 'error', message: 'Error while receiving order', data: error }));
        });
    });
