...
class AnalyticsService {
    ...
    static async getMenuItemAnalytics(days = 30) {
        try {
            ...
            ordersSnapshot.forEach(doc => {
                ...
                order.items.forEach(item => {
                    if (menuItems[item.id]) {
                        menuItems[item.id].totalQuantity = (menuItems[item.id].totalQuantity || 0) + item.quantity;
                        menuItems[item.id].totalRevenue = (menuItems[item.id].totalRevenue || 0) + (item.price * item.quantity);
                    }
                });
            });
            ...
            const topItemsByQuantity = [...menuItemsArray].sort((a, b) => b.totalQuantity - a.totalQuantity).slice(0, 10);
            const bottomItemsByQuantity = [...menuItemsArray].sort((a, b) => a.totalQuantity - b.totalQuantity).slice(0, 10);
            ...
            const averagePreparationTime = preparationTimeCount > 0 ?
                totalPreparationTime / preparationTimeCount : 0;
        ...
        try {
            ...
            reservations.forEach(reservation => {
                ...
                reservationsByDayOfWeek[dayOfWeek] =
                    (reservationsByDayOfWeek[dayOfWeek] || 0) + 1;
            ...
            const cancellationRate = reservationsByStatus[ReservationStatus.CANCELLED] / totalReservations;
    ...
    static async getOrderStatusAnalytics(days = 30) {
        try {
            ...
            const averageCompletionTime = completedOrders > 0 ?
                totalCompletionTime / completedOrders / (1000 * 60) : 0;
    ...
    static async getCustomerAnalytics(days = 90) {
        try {
            ...
            const returning = totalCustomers > 0 ?
    ...
    static async getOperationalAnalytics() {
        try {
            ...
            const todayReservationsSnapshot = await db.collection("reservations")
            ...
            const tableUtilizationRate = totalTables > 0 ? (tablesWithReservations / totalTables) * 100 : 0;

            
            const pendingOrders = activeOrders.filter(o =>
                o.status === OrderStatus.PENDING_PAYMENT || o.status === OrderStatus.PAID).length;
            const inProgressOrders = activeOrders.filter(o => o.status === OrderStatus.IN_PROGRESS).length;
            const readyOrders = activeOrders.filter(o =>
                o.status === OrderStatus.READY_FOR_PICKUP || o.status === OrderStatus.OUT_FOR_DELIVERY).length;
