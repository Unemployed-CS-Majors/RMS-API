...
class OrderService {
  ...
  static async validateOrderItems(orderItems) {
    try {
      ...
      const querySnapshot = await db.collection("menuItems").where("__name__", "in", itemIds).get();
  ...
  static async processPayment(order, successUrl, cancelUrl) {
    try {
      ...
      const session = await stripeService.createCheckoutSession(order, successUrl, cancelUrl);
