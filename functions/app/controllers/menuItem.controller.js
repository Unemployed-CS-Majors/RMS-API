...
class MenuItemController {
    static async createMenuItem(req, res) {
        try {
            ...
            if (!validationResult.isValid) {
                return res.status(400).json(createResponse('error', 'Validation failed', validationResult.errors));
            }
    ...
    static async getAllMenuItems(req, res) {
        try {
            ...
            res.status(200).json(createResponse('success', 'Menu items retrieved successfully', menuItems));
        } catch (error) {
    ...
    static async updateMenuItem(req, res) {
        try {
            ...
            const updatedData = {
                ...
                avgWaitTime: req.body.avgWaitTime ? parseInt(req.body.avgWaitTime) : currentMenuItem.avgWaitTime,
                allergens: req.body.allergens ?
                    (typeof req.body.allergens === 'string' ?
                        JSON.parse(req.body.allergens) : req.body.allergens)
            ...
            res.status(200).json(createResponse('success', 'Menu item updated successfully', updatedMenuItem));
        } catch (error) {
    ...
    static async getEnumValues(req, res) {
        try {
            res.status(200).json(createResponse('success', 'Enum values retrieved successfully', {
            ...
            res.status(500).json(createResponse('error', 'Failed to retrieve enum values', error.message));
        }
    }
}
