...
class ReservationController {
  ...
  static async cancelReservation(req, res) {
    ...
    if (!reservationId) {
      return res.status(400).json(createResponse("error", "Reservation ID is required", null));
    }
    ...
    try {
      ...
      if (reservation === null) {
        return res.status(404).json(createResponse("error", "Reservation not found", null));
      }
  ...
  static async rescheduleReservation(req, res) {
    ...
    if (validateTimeFormat(startTime) || validateTimeFormat(endTime)) {
      ...
      return res.status(400).json(createResponse("error", "Invalid time format", null));
    }

    try {
      const updateReservationStatus = await ReservationService.rescheduleReservation(
        reservationId,
        tableId,
        startTime,
        endTime,
  ...
  static async getFreeTableForGivenTime(req, res) {
    ...
    if (!startTime || !endTime || !seats) {
      return res.status(400).json(createResponse("error", "Start and end time are required", null));
    }
    ...
    if (validateSeats(seats)) {
      return res.status(400).json(createResponse("error", "Invalid seats", null));
    }
  ...
  static async getReservationByStatus(req, res) {
    ...
    if (!status) {
      return res.status(400).json(createResponse("error", "Status is required", null));
    }
