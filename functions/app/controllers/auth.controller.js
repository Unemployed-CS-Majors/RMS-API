...
const {
    validateRegister, validateLogin, validateRefreshToken,
} = require("../validators/auth.validators");
const {User, Privileges} = require("../models/user.model");
const {createResponse} = require("../utils/response.utils");
const {
    FIREBASE_REFRESH_TOKEN_URL, FIREBASE_SIGN_IN_ENDPOINT,
...
class AuthController {
     ...
     * @returns {Promise<void>} The result of the registration process.
     */
            ...
            .then((userRecord) => {
                ...
                return db
                    ...
                    .then(() => {
                        ...
                        res
                            .status(201)
                            .json(createResponse("success", "User registered successfully", {
                                uid: userRecord.uid,
                            }));
                    });
            })
     ...
     * @param {Object} req - The request object.
     * @param {Object} res - The response object.
     * @returns {Promise<void>} The result of the login process.
     */
    static login(req, res) {
        ...
        const signInEndpoint = FIREBASE_SIGN_IN_ENDPOINT;
     ...
     * @param {Object} req - The request object.
     * @param {Object} res - The response object.
     * @returns {Promise<void>} The result of the token refresh process.
     */
        ...
        axios
            ...
            .then((response) => {
                res.status(200).json(createResponse("success", "Token refreshed successfully", {
                    idToken: response.data.id_token, refreshToken: response.data.refresh_token,
                }));
            })
            .catch((error) => {
                console.error("Error:", error.response ? error.response.data : error.message);
